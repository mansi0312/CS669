...
--Obtain the Dollar to British Pound ratio, which gives us 0.67.
SELECT us_dollars_to_currency_ratio
FROM Currency
WHERE currency_name = 'Britsh Pound';
--Hardcode 0.67 in another query to get the price.
SELECT price_in_us_dollars * 0.67 AS price_in_british_pound
FROM Product
...
(SELECT us_dollars_to_currency_ratio
FROM Currency
WHERE currency_name = 'Britsh Pound') AS price_in_british_pound
FROM Product
WHERE product_name = 'Digital Thermometer'
...
(SELECT us_dollars_to_currency_ratio
FROM Currency
WHERE currency_name = 'Euro') AS price_in_euro
FROM Product
WHERE price_in_us_dollars * 
(SELECT us_dollars_to_currency_ratio FROM Currency WHERE currency_name = 'Euro') <25 OR price_in_us_dollars * 
(SELECT us_dollars_to_currency_ratio FROM Currency WHERE currency_name = 'Euro') >300 

--WRONG
SELECT Store_location.store_name,
Product.product_name, Alternate_name.name AS alternate_name,
...
HAVING COUNT(Offers.shipping_offering_id) > 0)
...
HAVING COUNT(Sells.store_location_id) = 5) locations
...
JOIN Sells ON Sells.store_location_id = Store_location.store_location_id
JOIN Product ON Product.product_id = Sells.product_id
WHERE Store_location.store_location_id = Store_location.store_location_id)